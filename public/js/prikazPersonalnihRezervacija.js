...
const pretragaRezervacija = async (query, pocetniDatum, krajnjiDatum, status) => {
  try {
    ...
    const res = await axios.get(`/rezervacije/personalniPodatci?query=${query}&datumKreacije[gte]=${pocetniDatum}&datumKreacije[lte]=${krajnjiDatum}&status=${status}`);
    ...
    if(noveRezervacije.length > 0){
      ...
      let recenzijaFormaBtn = $('.recenzijaFormaBtn');
      recenzijaFormaBtn.on('click', e => {
        e.preventDefault();
    
        const id = e.currentTarget.value;
        kreiranjeRecenzije(id);
      });
