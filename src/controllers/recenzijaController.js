...
exports.podatciZaSpremanjeRecenzije = catchAsync(async (req, res, next) => {
  ...
  let ponuda = await Ponuda.find({
    _id: rezervacija.ponuda._id,
    status: "aktivno",
  });
  ponuda = ponuda[0];
  ...
  for (let el in ponuda.pocetniDatumi) {
    if (
      new Date(ponuda.pocetniDatumi[el]) >=
      new Date(rezervacija.datumPocetkaPonude)
    ) {
      if (new Date(ponuda.pocetniDatumi[el]) < new Date()) {
        ...
        return next(
          new AppError(
            "Niste jos iskusili ponudu, ne možete davati recenziju!",
            400
          )
        );
      }
  ...
  if (rezervacija.recenzija) {
    const doc = await Recenzija.findByIdAndUpdate(
      rezervacija.recenzija.id,
      req.body,
      {
        new: true,
        runValidators: true,
