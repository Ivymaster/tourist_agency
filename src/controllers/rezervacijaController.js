...
const stripe = require("stripe")(
  "sk_test_51HftM2Hr1ZCotBHuIRN77jUjC4CIr2W6foPrVDo576MOZOgjtaTSGjbEtuohXkfjVRVcPpPppdD8xzJhB4Zo7m8w00GJEjtJ64"
);
...
exports.getMojeRezervacije = catchAsync(async (req, res, next) => {
  const rezervacije = await Rezervacija.find({ korisnik: req.user.id })
    .sort([
      ["datumPocetkaPonude", -1],
      ["datumKreacije", -1],
    ])
...
exports.podatciZaKreacijuRezervacije = catchAsync(async (req, res, next) => {
  ...
  for (let el in ponuda.pocetniDatumi) {
    if (
      new Date(ponuda.pocetniDatumi[el]) > new Date() &&
      new Date(ponuda.pocetniDatumi[el]).toString() ==
        new Date(req.body.datum).toString()
    ) {
      ponuda.brojSlobodnih[el] = ponuda.brojSlobodnih[el] - brojKarata;
    }
    if (ponuda.brojSlobodnih[el] < 0) {
      return next(
        new AppError("Broj karata premašuje ponuđene vrijednosti!", 400)
      );
    }
  }
  ...
  req.body.cijena = parseInt(
    ponuda.krajnjaCijena * req.body.brojKarata + req.body.dodatak
  );
  req.body.brojKarata = brojKarata;
  req.body.status = "rezervirano";
  (req.body.datumPocetkaPonude = req.body.datum), next();
});
...
exports.azuriranjeSlobodnihMjestaPonude = catchAsync(async (req, res, next) => {
  ...
  if (rezervacija.status == "placeno") {
    return next(
      new AppError("Rezervacija je vec placena, brisanje je onemoguceno!", 400)
    );
  }
  ...
  for (let el in ponuda.brojSlobodnih) {
    if (
      new Date(rezervacija.datumKreacije) < new Date(ponuda.pocetniDatumi[el])
    ) {
      ponuda.brojSlobodnih[el] =
        ponuda.brojSlobodnih[el] + rezervacija.brojKarata;
      break;
    }
  }

  await Ponuda.findByIdAndUpdate(rezervacija.ponuda.id, {
...
const kreiranjePlaceneRezervacije = async (session, req) => {
  ...
  if (rezervacijaId != "undefined") {
    ...
    if (rezervacija) {
      ...
      const noviKorisnik = await User.findOne({
  ...
  const korisnikId = (await User.findOne({ email: session.customer_email })).id;
  ...
  const noviKorisnik = await User.findOne({ email: session.customer_email });
  ...
  const novaRezervacijaURL = `${req.protocol}://${req.get("host")}/rezervacije`;
...
exports.getPodatciRezervacije = catchAsync(async (req, res, next) => {
  ...
  if (!korisnik && !ponuda) {
    return next(
      new AppError("Ne postoji korisnik ili ponuda sa ovim imenom!", 400)
    );
  }
...
exports.getVlasnikRezervacije = catchAsync(async (req, res, next) => {
  const korisnik = await User.find({ korisnickoIme: req.query.korisnickoIme });

  if (!korisnik) {
    return next(
      new AppError("Ne postoji korisnik sa tim korisnickim imenom!", 400)
    );
  }
  ...
  return res.redirect(
    "/korisnici?korisnickoIme=" + req.query.korisnickoIme + ""
  );
});
